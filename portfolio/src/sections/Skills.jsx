      ...
      >
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {" "}
            {/* Main grid for the two large skill categories */}
            {Object.entries(mainSkillCategories).map(
              ([mainCategory, skills]) => (
                <div
                  key={mainCategory}
                ...
                >
                  {" "}
                  {/* Large card for each main category */}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 justify-items-center">
                    ...
                    {skills.map((skill, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="group relative flex flex-col items-center justify-center bg-gray-700 rounded-full shadow-lg transform hover:scale-105 transition duration-300 cursor-pointer w-28 h-28"
                        >
                          <svg
                          ...
                          <div className="relative z-10 flex flex-col items-center justify-center">
                            <skill.icon
                              className={`text-3xl mb-1 ${skill.color} transition-colors duration-300 group-hover:text-gray-300`}
                            />
                    ...
                    <div className="mt-8 text-center">
                      ...
                      <ul className="list-none space-y-2">
                        {awsServices.map((service, idx) => (
                          <li
                            key={idx}
                            className="bg-gray-700 py-2 rounded-md text-white"
                          >
                            {service}
                          </li>
                        ))}
